...
/**
 * @file crops.js
 * @description Crops recipe Mystical Agriculture script|Fatebound: Scourge of the Damned
 * @license Apache-2.0
 * @author GoldenDev
 *
...
ServerEvents.recipes(event => {
  ...
  for (const Crop of CropList) {
    ...
    if (Crop.isEnabled()) {
      ...
      event.remove({
        id: `mysticalagriculture:seed/infusion/${CropName}`
      })
    }
  }
  ...
  if (Platform.isLoaded('botanypots')) {
    ...
    for (const seed of JsonExport.enabled) {
      if (!seenSeeds.includes(seed)) {
        ...
        let drops = [{
          chance: 1.0,
          output: Ingredient.of(Crop.getEssenceItem()).toJson()
        }]
        if (SecondarySeed > 0 && Crop.getTier().hasSecondarySeedDrop()) {
          drops.push({
            chance: SecondarySeed,
